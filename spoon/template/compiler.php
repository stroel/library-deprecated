		...
		while(1)
		{
			...
			if(preg_match_all($pattern, $content, $matches, PREG_SET_ORDER))
			{
				...
				foreach($matches as $match)
				{
					...
					// replace with
					$replace[] = '<?php
					if(isset(' . $variable . ') && ((is_array(' . $variable . ') && !empty(' . $variable . ')) || (!is_array(' . $variable . ') && (' . $variable . ' !== \'\') && (' . $variable . ' !== false))))
					{
						?>';
					$replace[] = '<?php } ?>';

					// inverse option
					$replace[] = '<?php if(!isset(' . $variable . ') || (is_array(' . $variable . ') && empty(' . $variable . ')) || (!is_array(' . $variable . ') && ((' . $variable . ' === \'\') || (' . $variable . ' === false)))): ?>';
					$replace[] = '<?php endif; ?>';
