        ...
        foreach($records as $i => &$record)
        {
            ...
            foreach($sequence as $name)
            {
                ...
                else
                {
                    ...
                    {
                        // open url tag
                        $columnValue .= '<a href="' . str_replace($record['labels'], $record['values'], (string) $column->getURL()) . '"';

                        // add title
                        $columnValue .= ' title="' . str_replace($record['labels'], $record['values'], (string) $column->getURLTitle()) . '"';

                        // confirm
                        if($column->getConfirm() && $column->getConfirmMessage() !== null)
                        {
                            // default confirm
                            if($column->getConfirmCustom() == '') $columnValue .= ' onclick="return confirm(\'' . str_replace($record['labels'], $record['values'], (string) $column->getConfirmMessage()) . '\');"';

                            // custom confirm
                            else
                            {
                                // replace the message
                                $tmpValue = str_replace('%message%', $column->getConfirmMessage(), (string) $column->getConfirmCustom());

                                // make vars available
                                $tmpValue = str_replace($record['labels'], $record['values'], (string) $tmpValue);
                    ...
                    {
                        // open img tag
                        $columnValue .= '<img src="' . str_replace($record['labels'], $record['values'], (string) $column->getImage()) . '"';

                        // add title & alt
                        $columnValue .= ' alt="' . str_replace($record['labels'], $record['values'], (string) $column->getImageTitle()) . '"';
                        $columnValue .= ' title="' . str_replace($record['labels'], $record['values'], (string) $column->getImageTitle()) . '"';
    ...
    private function parseRecord(array $record)
    {
        ...
        foreach($this->columns as $column)
        {
            ...
            if(!in_array('[' . $column->getName() . ']', $array['labels']))
            {
                ...
                $array['values'][] = str_replace($array['labels'], $array['values'], (string) $column->getValue());
