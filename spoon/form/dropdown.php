...
class SpoonFormDropdown extends SpoonFormAttributes
{
	...
	protected function getAttributesHTML(array $variables)
	{
		...
		foreach($this->attributes as $key => $value)
		{
			...
			// other elements
			else $html .= ' ' . $key . '="' . str_replace(array_keys($variables), array_values($variables), (string) $value) . '"';
		}
	...
	public function parse(SpoonTemplate $template = null)
	{
		...
		if($template !== null)
		{
			$template->assign('ddm' . SpoonFilter::toCamelCase(str_replace('[]', '', (string) $this->attributes['name'])), $output);
			$template->assign('ddm' . SpoonFilter::toCamelCase(str_replace('[]', '', (string) $this->attributes['name'])) . 'Error', ($this->errors != '') ? '<span class="formError">' . $this->errors . '</span>' : '');
		}
